...
def reverse_list(items):
    ...
    **Do not use** the python function `reversed()` or the method
    ...
    #slice the whole list backwards!
    new_list = items[::-1]
...
def duplicates(items):
    ...
    #now they are in numeric order so I can compare "this" one to the previous one
    items.sort()
    prior_one = ''

    for item in items:
        if item == prior_one:
            my_duplicates.append(item)
        prior_one = item

        #convert to a set to remove duplicates
        my_duplicates = set(my_duplicates)
        #convert back to a list to return a list
        my_duplicates = list(my_duplicates)
